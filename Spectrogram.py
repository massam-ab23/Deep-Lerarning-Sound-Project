...
The training loop calculates accuracy and loss per epoch and plots the final metrics, including a confusion matrix, to visualize model performance.
'''
...
import parameters  # Imports class list and batch size parameters
...
class SoundDataset(Dataset):
    def __init__(self, csv_file, img_dir, transform=None, num_samples=1000):
        ...
        self.labels = self.labels.sample(n=num_samples, replace=True, random_state=42).reset_index(drop=True)  # Randomly sample 1000 entries
        self.img_dir = img_dir  # Directory for images
        self.transform = transform  # Image transformations (resize, normalize)

        # Label-to-index mapping from CLASS_LIST in parameters
    ...
    def __getitem__(self, idx):
        ...
        label = self.label_to_idx[label]  # Mapping label to index
...
transform = transforms.Compose([
    ...
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])  # Normalizing for ImageNet weights
])

# Setting paths for CSV and image directory
train_csv = "data/train_labels.csv"
img_dir = "data/Spectrogram_images"
...
vgg19.classifier = nn.Sequential(
    ...
    nn.ReLU(),
    nn.Dropout(0.63),
    nn.Linear(4096, parameters.NUM_CLASSES),  # Output layer for number of classes
    nn.LogSoftmax(dim=1)
)
...
# Define the learning rate directly here
LEARNING_RATE = 0.0002  # Setting learning rate for optimizer
...
def train_model():
    ...
    for epoch in range(0, NUM_EPOCHS):
        ...
        vgg19.train()  # Setting model to training mode
        running_loss = 0.0
        correct_train = 0.0
        total_train = 0.0
    ...
    # Plotting final confusion matrix after training
