...
# Generate and save a standard spectrogram image from an audio file
def save_spectrogram_image(audio_path, output_image_path):
    y, sr = librosa.load(audio_path)  # Load the audio file
    S = librosa.stft(y)  # Compute the Short-Time Fourier Transform
    S_db = librosa.amplitude_to_db(abs(S))  # Convert amplitude to decibels

    plt.figure(figsize=(10, 4))
    librosa.display.specshow(S_db, sr=sr, x_axis='time', y_axis='log')  # Display as spectrogram
    plt.colorbar(format='%+2.0f dB')  # Add color bar for decibel levels
    plt.title('Spectrogram')
    plt.tight_layout()
    plt.savefig(output_image_path)  # Save the spectrogram as an image file
    plt.close()

# Generate and save a mel-spectrogram image from an audio file
def save_mel_spectrogram_image(audio_path, output_image_path):
    ...
    plt.figure(figsize=(10, 4))
    librosa.display.specshow(S_db, sr=sr, x_axis='time', y_axis='mel')  # Display mel-spectrogram
    plt.colorbar(format='%+2.0f dB')  # Add color bar for decibel levels
    plt.title('Mel-Spectrogram')
    plt.tight_layout()
    plt.savefig(output_image_path)  # Save the mel-spectrogram as an image file
    plt.close()

# Generate and save an MFCC (Mel-Frequency Cepstral Coefficients) image from an audio file
def save_mfcc_image(audio_path, output_image_path):
    y, sr = librosa.load(audio_path)  # Load the audio file
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13)  # Compute 13 MFCCs

    plt.figure(figsize=(10, 4))
    librosa.display.specshow(mfcc, sr=sr, x_axis='time')  # Display MFCC
    plt.colorbar()  # Add color bar
    plt.title('MFCC')
    plt.tight_layout()
    plt.savefig(output_image_path)  # Save the MFCC as an image file
    plt.close()
...
def process_audio_to_images(audio_dir, spectrogram_dir, mel_spectrogram_dir, mfcc_dir):
    ...
    for filename in os.listdir(audio_dir):
        if filename.endswith('.wav'):  # Process only .wav files
            files_found = True
            audio_path = os.path.join(audio_dir, filename)  # Get full path to audio file
            base_filename = os.path.splitext(filename)[0]  # Remove file extension for image naming
    ...
    if not files_found:
        print("No .wav files found in the directory.")  # Inform if no .wav files were found

# Update these paths to point to your correct directories for test data
...
# Process the test audio files and save the images
